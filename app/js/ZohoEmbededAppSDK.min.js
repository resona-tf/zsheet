var ZOHO = function () {
	  ...
	  CRM: function () {
		...
		function p(a, d, e) {
		  ...
		  if (e) f = d;
			...
			if (c) {
			  var x, k = [];
			  for (x in c) k.push(encodeURIComponent(x) + "=" + encodeURIComponent(c[x]));
			  x = k.join("&");
			  f += (-1 < f.indexOf("?") ? "&" : "?") + x
			}
			f = {
		...
		return {
		  ...
		  CONNECTION: {
			invoke: function (a, d) {
			  ...
			  f.RESPONSE_TYPE = d.RESPONSE_TYPE;
			  e.data = f;
			  return b({
  ...
  var ZSDKUtil = function (c) {
	  ...
	  function h(b) {
		...
		b.substr(b.indexOf("?") + 1).split("&").forEach(function (b, n) {
		  var h = b.split("=");
		  c[h[0]] = h[1]
		});
	...
	ZSDKMessageManager = function (c) {
	  function l(e) {
		...
		try {
		  ...
		  r.Error("[SDK.MessageHandler] => ", c.stack)
		}
	  }
  ...
  function ZSDK() {
	function c() {
	  "function" !== typeof k ? z.Error("No OnLoad Handler provided to execute.") : C ? z.Error("OnLoad event already triggered.") : (k.call(q, q), C = !0)
	}
